...
状态数据库选项包括 LevelDB 和 CouchDB 。LevelDB 是默认嵌入在节点（peer）程序中的键-值数据库。 CouchDB 是一个可选的额外扩展的状态数据库。就像 LevelDB 键-值存储一样，CouchDB 可以存储任何在 链码中建模的二进制数据（CouchDB 附件功能在内部用于非 JSON 二进制数据）。但是作为一个 JSON 文 档存储数据库，当链码值（比如，资产）以 JSON 数据建模时， CouchDB 额外支持链码数据的富查询。
...
LevelDB 和 CouchDB 都支持链码操作，比如获取或者设置一个键（资产），以及基于键查询。键可以按范 围查询，以复合键建模就可以支持针对多个参数的等效查询。比如一个复合键 ``owner,asset_id`` 就可 以查询一个实体的所有资产。这些基于键的查询可以用来在账本上做只读查询，同样可以用于在交易中更新 账本
